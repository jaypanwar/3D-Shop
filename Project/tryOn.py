...
def add_sprite(img):
    ...
    put_sprite(int(img.rsplit('/',1)[0][-1])) #here it will return the integer value from 1 to 6, which will denotes that which catagory is the apparel belongs to i.e either frock or tops or etc.
...
def cvloop(run_event):
    ...
    while run_event.is_set(): 
        ...
        for face in faces: 
            ...
            if SPRITES[3]:#Tiara
                apply_sprite(image, image_path,w+45,x-20,y+20, incl, ontop = True)

            #Necklaces
            if SPRITES[1]:
                (x1,y1,w1,h1) = get_face_boundbox(shape, 6)
                apply_sprite(image, image_path,w1,x1,y1+150, incl,  ontop = False)
            ...
            if SPRITES[2]:
                ...
                apply_sprite(image, image_path,w3,x3-13,y3+30, incl,ontop=False)
            ...
            if SPRITES[5]:
                ...
                apply_sprite(image, image_path,w1+580,x1-350,y1+80, incl,  ontop = False)
            ...
            if SPRITES[4]:
                ...
                (x1,y1,w1,h1) = get_face_boundbox(shape, 8)
                apply_sprite(image, image_path,w1+350,x1-230,y1+100, incl,  ontop = False)
