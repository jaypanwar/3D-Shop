    ...
    for c in range(3):
        try:
            frame[y_offset:y_offset+h, x_offset:x_offset+w, c] =  \
                sprite[:, :, c] * (sprite[:, :, 3]/255.0) + frame[y_offset:y_offset+h,
                                                                  x_offset:x_offset+w, c] * (1.0 - sprite[:, :, 3]/255.0)
...
def detectUpperBody(image):
    ...
    Rect = cascade.detectMultiScale(
        imageGray, scaleFactor=1.1, minNeighbors=1, minSize=(1, 1))
    if len(Rect) <= 0:
        return False
    else:
        return Rect
...
def add_sprite(img):
    ...
    # here it will return the integer value from 1 to 6, which will denotes that which catagory is the apparel belongs to i.e either frock or tops or etc.
...
def cvloop(run_event):
    ...
    while run_event.is_set():
        ...
        for face in faces:
            ...
            if SPRITES[3]:  # Tiara
                apply_sprite(image, image_path, w+45, x -
                             20, y+20, incl, ontop=True)

            # Necklaces
            if SPRITES[1]:
                (x1, y1, w1, h1) = get_face_boundbox(shape, 6)
                apply_sprite(image, image_path, w1, x1,
                             y1+150, incl,  ontop=False)
            ...
            if SPRITES[2]:
                ...
                apply_sprite(image, image_path, w3, x3 -
                             13, y3+30, incl, ontop=False)
                (x3, y3, w3, h3) = get_face_boundbox(shape, 8)  # nose
                apply_sprite(image, image_path, w3, x3+7, y3+47, incl)

                # filza laptop
                # (x3,y3,w3,h3) = get_face_boundbox(shape, 7) #nose
                # apply_sprite(image, image_path,w3,x3-40,y3+30, incl,ontop=False)
                # (x3,y3,w3,h3) = get_face_boundbox(shape, 8) #nose
                # apply_sprite(image, image_path,w3,x3+40,y3+75, incl)
            ...
            if SPRITES[5]:
                ...
                apply_sprite(image, image_path, w1+580, x1 -
                             350, y1+80, incl,  ontop=False)
            ...
            if SPRITES[4]:
                ...
                (x1, y1, w1, h1) = get_face_boundbox(shape, 8)
                apply_sprite(image, image_path, w1+350, x1 -
                             230, y1+100, incl,  ontop=False)
                # apply_sprite(image, image_path,w1+380,x1-230,y1+100, incl,  ontop = False)
                # apply_sprite(image, image_path,w1+380,x1-230,y1+150, incl,  ontop = False)
...
def try_on(image_path):
    btn1 = Button(root, text="Try it ON",
                  command=lambda: add_sprite(image_path))
